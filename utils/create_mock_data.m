...
%% PART 2: Second 2 minutes - Clear ENTRAINMENT to the 1Hz stimulus
...
for ch = 1:n_channels
    ...
    % KEY CHANGE: Add entrainment by including a component of the 1Hz sine wave
    % The entrainment strength varies by channel (stronger in frontal channels)
    entrainment_strength = 0.2 + 0.6 * (1 - min(1, ch/n_channels)); % Higher for lower channel numbers
    
    % Add phase-locked component to create entrainment
    entrained_signal = baseline + entrainment_strength * sine_wave_segment2;
...
% Combine all channels including the 65th sine wave channel
...
% Save the EEG data
save('nrem_sleep_with_entrainment_contrast.mat', 'EEG');

% Also save as EEGLAB .set file
pop_saveset(EEG, 'filename', 'nrem_sleep_with_entrainment_contrast.set');
